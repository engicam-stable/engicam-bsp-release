...
Download the BSP source:
    ...
    $: PATH=${PATH}:~/bin
    $: mkdir -p ~/yocto/scarthgap_v24.12-st 
    $: cd ~/yocto/scarthgap_v24.12-st 
    $: curl https://raw.githubusercontent.com/engicam-stable/engicam-bsp-release/scarthgap_v24.12-st /Dockerfile > Dockerfile
    $: repo init -u https://github.com/engicam-stable/engicam-bsp-release.git -b scarthgap_v24.12-st  -m engicam-bsp-release.xml
    $: repo sync
...
where ``<machine-name>`` corresponds to the module for which the operative system image will be compiled and ``<build-dir-name>`` is the bulding directory name chosen by the user, as in the following example:
	 ...
	 DISTRO=openstlinux-weston MACHINE=stm32mp25d-icore.conf source layers/meta-engicam-st/scripts/envsetup.sh build
...
Once the image is compiled it will be possible to find in the build directory a deploy folder with the image files. The relative path to this folder from the yocto directory will be:
	...
	tmp-glibc/deploy/images/stm32mp25d-icore
